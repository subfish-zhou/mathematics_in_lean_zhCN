...
在Lean中, 陈述一个定理等价于确立一个证明该定理的目标. Lean提供`rewrite`策略, 缩写为`rw`, 用于在目标中实现等式代换, 把目标中出现在等式左边的项代换成右边的项. 如果`a`, `b`, `c`是实数, `mul_assoc a b c`是乘法结合律等式`a * b * c = a * (b * c)`, `mul_comm a b`是乘法交换律等式`a * b = b * a`. 在Lean中, 乘法是左结合的, 因此`mul_assoc`的左边也可以写为`（a * b）* c`. 然而通常情况下, 注意Lean的符号约定并在Lean也这样做时省略括号是很好的范式. 
